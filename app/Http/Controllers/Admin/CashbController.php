...
class CashbController extends Controller
{
    ...
    public function cashbList(Request $request)
    {
        ...
        $wheredr[] = ['users_wallet_out.create_time', '>=', strtotime(date('Y-m-d') . " 0:0:0")];
        $wheredr[] = ['users_wallet_out.create_time', '<=', strtotime(date('Y-m-d') . " 23:59:59")];
        $drlist = DB::table('users_wallet_out')
            ->join('users', 'users.id', '=', 'users_wallet_out.user_id')
            ->join('currency', 'currency.id', '=', 'users_wallet_out.currency')
            ->leftjoin('user_cash_info', 'user_cash_info.user_id', '=', 'users_wallet_out.user_id')
            ->select('users_wallet_out.*','user_cash_info.bank_account','currency.price','currency.rmb_relation', 'users.account_number', 'currency.name')
            ->where($wheredr)
            ->get();
        ...
        $wheredy[] = ['users_wallet_out.create_time', '>=',$thismonth_start];
        $wheredy[] = ['users_wallet_out.create_time', '<=', $thismonth_end];
        $dylist = DB::table('users_wallet_out')
            ->join('users', 'users.id', '=', 'users_wallet_out.user_id')
            ->join('currency', 'currency.id', '=', 'users_wallet_out.currency')
            ->leftjoin('user_cash_info', 'user_cash_info.user_id', '=', 'users_wallet_out.user_id')
            ->select('users_wallet_out.*','user_cash_info.bank_account','currency.price','currency.rmb_relation', 'users.account_number', 'currency.name')
        ...
        return $this->layuiData($userWalletOutList,'{"dr":"'.$dr.'","dy":"'.$dy.'"}');
    }
