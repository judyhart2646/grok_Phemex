...
class UserRealController extends Controller
{
    ...
    public function d_detail(Request $request){
        $id = $request->get('id', 0);
        if (empty($id)) {
        ...
        return view('admin.userReal.d_info', ['result' => $result,'imageServerUrl' => $imageServerUrl]);
    }
    ...
    public function dauth(Request $request){
        ...
        $userreal = UserReal::find($id);
        if (empty($userreal)) {
            return $this->error("认证信息未找到");
        }
        $front_pic = $request->get('front_pic');
        $reverse_pic = $request->get('reverse_pic');
        
        if(empty($front_pic) || empty($reverse_pic)){
            return $this->error("照片必填项");
        }
        try {
