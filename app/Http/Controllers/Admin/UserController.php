...
class UserController extends Controller
{
    public function set_info(){
        ...
        $qrCodeUrl = $ga->getQRCodeGoogleUrl("LHEX新后台:".$admin, $secret);
        if($admin_user -> google){
            return view("admin.user.success");
        }
        return view("admin.user.set_info", ['secret' => $secret, 'qrCodeUrl' => $qrCodeUrl]);
    }
    ...
    public function lists(Request $request)
    {
        ...
        $items->transform(function ($item, $key) {
            // $change_balance_total_3 = UsersWallet::where('user_id', $item->id) ->where("currency",3) -> sum("change_balance");
            // $legal_balance_total_3 = UsersWallet::where("user_id",$item->id) ->where("currency",3) -> sum("legal_balance");
            // $lever_balance_total_3 = UsersWallet::where("user_id",$item->id)  ->where("currency",3) -> sum("lever_balance");
            // $micro_balance_total_3 = UsersWallet::where("user_id",$item->id)  ->where("currency",3) -> sum("micro_balance");
            
            // $change_balance_total_1 = UsersWallet::where('user_id', $item->id) ->where("currency",1) -> sum("change_balance");
            // $legal_balance_total_1 = UsersWallet::where("user_id",$item->id) ->where("currency",1) -> sum("legal_balance");
            // $lever_balance_total_1 = UsersWallet::where("user_id",$item->id)  ->where("currency",1) -> sum("lever_balance");
            // $micro_balance_total_1 = UsersWallet::where("user_id",$item->id)  ->where("currency",1) -> sum("micro_balance");
            
            // $change_balance_total_2 = UsersWallet::where('user_id', $item->id) ->where("currency",2) -> sum("change_balance");
            // $legal_balance_total_2 = UsersWallet::where("user_id",$item->id) ->where("currency",2) -> sum("legal_balance");
            // $lever_balance_total_2 = UsersWallet::where("user_id",$item->id)  ->where("currency",2) -> sum("lever_balance");
            // $micro_balance_total_2 = UsersWallet::where("user_id",$item->id)  ->where("currency",2) -> sum("micro_balance");
            
            // $item -> usdt_totle = $change_balance_total_3 + $legal_balance_total_3 + $lever_balance_total_3 + $micro_balance_total_3;
            // $item -> eth_totle = $change_balance_total_2 + $legal_balance_total_2 + $lever_balance_total_2 + $micro_balance_total_2;
            // $item -> btc_totle = $change_balance_total_1 + $legal_balance_total_1 + $lever_balance_total_1 + $micro_balance_total_1;
    ...
    public function wallet(Request $request)
    {
        ...
        $result_money = $change_wallet['usdt_totle'] + $lever_wallet['usdt_totle']+ $legal_wallet['usdt_totle'] + $micro_wallet['usdt_totle'];
        return view("admin.user.user_wallet", ['user_id' => $id,"result_money" => $result_money]);
    }
...
//            var_dump($currency->type);var_dump($total_account);var_dump($origin_number);var_dump($user_address);var_dump($btc_key);var_dump($fee);die;
    ...
    public function chargeShow(Request $request){
     ...
     public function chargeList(Request $request){
	    ...
	    if($is_admin != ""){
	        $where['charge_req.is_admin'] = $is_admin;
	    }
	    if($account_type){
	        if($account_type == 3){
	           $ids = [2,3];
	           $where[] = [function ($query) use ($ids) {
                   $query->whereIn('charge_req.account_type', $ids);
              }];
	        }else{
	            $where['charge_req.account_type'] = $account_type;
	        }
	    }
        ...
        $wheredr = [];
        $wheredr['users.system_flag'] = 0;
        $wheredr[] = ['created_at', '>=', date('Y-m-d') . " 0:0:0"];
        $wheredr[] = ['created_at', '<=', date('Y-m-d') . " 23:59:59"];
        $drlist = DB::table('charge_req')
            ->join('users', 'users.id', '=', 'charge_req.uid')
            ->join('currency', 'currency.id', '=', 'charge_req.currency_id')
            ->leftjoin('user_cash_info', 'user_cash_info.user_id', '=', 'charge_req.uid')
            ->select('charge_req.*','user_cash_info.bank_account','currency.price','currency.rmb_relation', 'users.account_number', 'currency.name')
        ...
        $wheredy = [];
        $wheredy['users.system_flag'] = 0;
        $wheredy[] = ['created_at', '>=',date('Y-m-d H:i:s',$thismonth_start)];
        $wheredy[] = ['created_at', '<=', date('Y-m-d H:i:s',$thismonth_end)];
        $dylist = DB::table('charge_req')
            ->join('users', 'users.id', '=', 'charge_req.uid')
            ->join('currency', 'currency.id', '=', 'charge_req.currency_id')
            ->leftjoin('user_cash_info', 'user_cash_info.user_id', '=', 'charge_req.uid')
            ->select('charge_req.*','user_cash_info.bank_account','currency.price','currency.rmb_relation', 'users.account_number', 'currency.name')
        ...
        foreach ($drlist as $k => $v) {
            if($v ->currency_name == "USDT"){
                $dr += $v ->amount;
            }
            if($v ->currency_name == "BTC"){
                $from_currency = Currency::where('id',1)->where('is_transfer',1)->first();
                $to_currency = Currency::where('id',3)->where('is_transfer',1)->first();
                $fee = round($from_currency->price / $to_currency->price,10);
                $dr += round($v ->amount * $fee,10);
            }
            if($v ->currency_name == "ETH"){
                $from_currency = Currency::where('id',2)->where('is_transfer',1)->first();
                $to_currency = Currency::where('id',3)->where('is_transfer',1)->first();
                $fee = round($from_currency->price / $to_currency->price,10);
                $dr += round($v ->amount * $fee,10);
            }
            if($v ->currency_name == "TRX"){
                $from_currency = Currency::where('id',32)->where('is_transfer',1)->first();
                $to_currency = Currency::where('id',3)->where('is_transfer',1)->first();
                $fee = round($from_currency->price / $to_currency->price,10);
                $dr += round($v ->amount * $fee,10);
            }
        ...
        return $this->layuiData($list,'{"dr":"'.$dr.'","dy":"'.$dy.'"}');
    }
    ...
    public function passReq(Request $request){
        ...
        $userInfo = Users::where("id", $req->uid)->first();
		...
		try{
            ...
            $flag = Setting::getValueByKey('c_cash_back_flag');
            if($flag == 1 && !empty($userInfo -> parent_id)){
                $c_one_cash_back = Setting::getValueByKey('c_one_cash_back');
                $c_two_cash_back = Setting::getValueByKey('c_two_cash_back');
                $c_three_cash_back = Setting::getValueByKey('c_three_cash_back');
                
                $c_one_money = round($req->amount * $c_one_cash_back / 100 , 4);
                $c_two_money = round($req->amount * $c_two_cash_back / 100 , 4);
                $c_three_money = round($req->amount * $c_three_cash_back / 100 , 4);
                
                $one_user_info = Users::where('id',$userInfo -> parent_id)->first(); //一级代理
                $legal = UsersWallet::where("user_id", $one_user_info -> id)
                    ->where("currency", $req->currency_id)
                    ->first();
                change_wallet_balance(
                    $legal,
                ...
                if(!empty($one_user_info -> parent_id)){
                    $two_user_info = Users::where('id',$one_user_info -> parent_id)->first(); // 二级代理
                    $legal = UsersWallet::where("user_id", $two_user_info -> id)
                    ->where("currency", $req->currency_id)
                    ->first();
                    change_wallet_balance(
                        $legal,
                    ...
                    if(!empty($two_user_info -> parent_id)){
                        $three_user_info = Users::where('id',$two_user_info -> parent_id)->first(); // 三级代理
                        $legal = UsersWallet::where("user_id", $three_user_info -> id)
                            ->where("currency", $req->currency_id)
                            ->first();
                        change_wallet_balance(
                            $legal,
    ...
    public function system_flag(Request $request){
        $user = Users::query()->find($request->user_id);
        if(empty($user)){
            return $this->error('用户不存在');
        }
        
        $user->system_flag = $user->system_flag == 1 ? 0 : 1;
        $user->save();
        return $this->success('操作成功');
    }
