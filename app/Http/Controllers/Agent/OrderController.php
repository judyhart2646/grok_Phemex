...
use App\{Agent, AgentMoneylog, Currency, LeverTransaction, TransactionComplete, TransactionOrder, Users,UsersWallet,CurrencyMatch,MicroOrder,LhDepositOrder};
...
class OrderController extends Controller
{
    public function deposit_order(){
        ...
        return view("agent.order.deposit_order")->with('currencies', $currencies)
            ->with('currency_matches', $currency_matches);
    }
    
    public function deposit_order_list(Request $request){
        ...
        $list = $model->join('users','users.id','=','user_id')->join('currency','currency.id','=','currency_id')
            ...
            })->where($where)->whereIn('user_id', $node_users)->orderBy('lh_deposit_order.created_at','desc')
    ...
    public function micro_order_edit(Request $request)
    {
        ...
        return view('agent.order.micro_order_edit', ['result' => $result]);
