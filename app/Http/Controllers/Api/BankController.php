...
//        }
    ...
    public function dispatchInterest(){
      $res = LhDepositOrder::where([
        ...
        foreach($res as $order){
            $time_new = date('H:i:s', strtotime($order->created_at));
            $end_at = $order->end_at;
            if(date('Y-m-d H:i:s', strtotime($end_at." ".$time_new)) <= date('Y-m-d H:i:s') && !empty($order -> last_settle_time)){
                if($order->last_settle_time >= date("Y-m-d")){
                    ...
                    DB::beginTransaction();
                    try{
                        $totalmoney = $order->amount + $order->total_interest;
                    ...
                    $flag = Setting::getValueByKey('w_cash_back_flag');
                    $userInfo = Users::where("id", $order->user_id)->first();
                    if ($flag == 1 && !empty($userInfo->parent_id)) {
                        $c_one_cash_back = Setting::getValueByKey('w_one_cash_back');
                        $c_two_cash_back = Setting::getValueByKey('w_two_cash_back');
                        $c_three_cash_back = Setting::getValueByKey('w_three_cash_back');
            
                        $c_one_money = round($totalmoney * $c_one_cash_back / 100, 4);
                        $c_two_money = round($totalmoney * $c_two_cash_back / 100, 4);
                        $c_three_money = round($totalmoney * $c_three_cash_back / 100, 4);
    ...
    public function myDepositOrder(Request $request){
        ...
        $res = LhDepositOrder::where('user_id',$user_id)
            ...
            ->skip($limit*($page-1))->take($limit)
            ->get(['currency.name as currency_name','lh_deposit_order.id','amount','day_rate','total_interest','start_at','end_at','created_at','status']);
        return $this->success([
            'order_list' => $res
        ]);
    }
