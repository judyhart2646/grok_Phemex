...
//        $user_id = Users::getUserId();
    ...
    public function levelList(){
        $list = UserLevelModel::orderBy('id', 'asc') -> paginate(999);
        return $this->success($list);
    }
    
    public function getTotalChargeAmountByUserId(){
        $user_id = Users::getUserId();
        $total = ChargeReq::where("uid",$user_id) -> where("status", 2) -> sum("amount");
        return $this->success($total);
    }
    
    public function getTeamList(){
        $user_id = Users::getUserId();
        $one_list=Users::where('parent_id',$user_id)->get()->toArray(); //一级代理
        $two_list = [];
        $one_list_ids = "";
        $three_list = [];
        ...
        $w_one_cash_back = Setting::getValueByKey('w_one_cash_back');
        $w_two_cash_back = Setting::getValueByKey('w_two_cash_back');
        $w_three_cash_back = Setting::getValueByKey('w_three_cash_back');
        foreach($one_list as $k=>$v){
            $one_invite_money = WalletLog::where('user_id',$user_id) -> where('from_user_id',$v['id']) -> sum('change');
            $one_list[$k]['invite_moeny'] = round($one_invite_money,4);
            $one_list_ids = $one_list_ids . $v['id'] . "," ;
        }
        $arr = explode(',', $one_list_ids);
        array_pop($arr);
        $one_ids = implode(',', $arr);
        $one_ids = explode(',', $one_ids);
        if(!empty($one_list)){
            $two_list=Users::whereIn('parent_id',$one_ids)->get()->toArray();; //二级代理
            $two_list_ids = "";
            foreach($two_list as $k=>$v){
                $two_invite_money = WalletLog::where('user_id',$user_id) -> where('from_user_id',$v['id']) -> sum('change');
                $two_list[$k]['invite_moeny'] = round($two_invite_money,4);
                $two_list_ids = $two_list_ids . $v['id'] . "," ;
            }
            $arr_2 = explode(',', $two_list_ids);
            array_pop($arr_2);
            $two_ids = implode(',', $arr_2);
            $two_ids = explode(',', $two_ids);
            if(!empty($two_list)){
                $three_list=Users::whereIn('parent_id',$two_ids)->get()->toArray();; //三级代理
                foreach($three_list as $k=>$v){
                    $three_invite_money = WalletLog::where('user_id',$user_id) -> where('from_user_id',$v['id']) -> sum('change');
                    $three_list[$k]['invite_moeny'] = round($three_invite_money,4);
                }
            }
        }
        $total_invite_moeny = WalletLog::where('user_id',$user_id) -> where('from_user_id','!=','0') -> sum('change');
        $total_invite_moeny = round($total_invite_moeny,4);
        $list=['one'=>$one_list,'two'=>$two_list,'three'=>$three_list,'total_invite_moeny'=>$total_invite_moeny,'c_one_cash_back'=>$c_one_cash_back,'c_two_cash_back'=>$c_two_cash_back,'c_three_cash_back'=>$c_three_cash_back,'w_one_cash_back'=>$w_one_cash_back,'w_two_cash_back'=>$w_two_cash_back,'w_three_cash_back'=>$w_three_cash_back];
        return $this->success(['data'=>$list]);
    }
}
?>