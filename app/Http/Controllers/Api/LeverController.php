...
//    }
    ...
    public function submit()
    {
        ...
        $status = Input::get('status', LeverTransaction::TRANSACTION); //默认是市价交易,为0则是挂单交易
        $target_price = Input::get('target_price', 0); //目标价格
        $target_profit_price = Input::get('target_profit_price', 0); //目标价格
        $stop_loss_price = Input::get('stop_loss_price', 0); //目标价格
        $now = time();
        $user_lever = 0;
...
//        $all_money = bc_mul($fact_price, $num, 5);
        ...
        //计算手续费
        $lever_trade_fee_rate = bc_div($currency_match->lever_trade_fee ?? 0, 100);
        $trade_fee = bc_mul( $lever_share_num * $share, $lever_trade_fee_rate);
        DB::beginTransaction();
        try {
            $legal = UsersWallet::where("user_id", $user_id)
                ->where("currency", $legal_id)
            ...
            $user_lever = $legal->lever_balance;
            $caution_money = $lever_share_num * $share; //保证金
            // $caution_money = bc_div($origin_price,$multiple,4);//保证金
            ...
            $lever_transaction->create_time = $now;
            $lever_transaction->status = $status;
            // var_dump($target_profit_price);
            // var_dump($stop_loss_price);
            if($target_profit_price > 0 && $stop_loss_price > 0){
                if($type == 1){
                 //买入
                    if ($target_profit_price <= $origin_price || $target_profit_price <= $last_price ) {
                        return $this->error('买入(做多)止盈价不能低于开仓价和当前价');
                    }
                    if ($stop_loss_price >= $origin_price || $stop_loss_price >= $last_price) {
