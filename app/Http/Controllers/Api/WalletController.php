...
class WalletController extends Controller
{
    ...
    public function postWalletOut()
    {
        ...
        $tx_fee_flag = Setting::getValueByKey('tx_fee_flag');
        try {
            DB::beginTransaction();
            ...
            $walletOut->rate = $rate;
            if($tx_fee_flag == 1){
                $walletOut->real_number = $number  - round($number * $rate / 100,4) ;
            }else{
    ...
    public function getWalletAddressIn()
    {
        ...
        if($currency_id==3){//usdt充值地址
            $address=[
                ...
                'erc20_back'=>$currencyInfo->address_erc_back ?? '', //erc20
                "trc20_back"=>$currencyInfo->address_omni_back ?? '' //trc20
            ];
        }else{
            $address=[
                'main' => $currencyInfo->address_erc ?? '',
                'back' => $currencyInfo->address_erc_back ?? '',
            ];
        }
        return $this->success($address);
    }
