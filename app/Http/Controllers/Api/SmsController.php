...
//                $this->setSmsIpKey($request->ip().$mobile, $mobile);
    ...
    public function sendModu(Request $request)
    {
        ...
        $area_code = $request->get('area_code', 86);
        $accesskey = "COGNpGhbcKwXWVUA7IvP3Q==";
        $secretkey = "eaaaec38a13549c797fce78a33246fb2";
        $phoneNumber = "$mobile";
    ...
    public function smsPaaSooSend(Request $request)
    {
        ...
        $verification_code = $this->createSmsCode(6);
        $content = $sms_signature . 'Your verification code is[' . $verification_code . ']';
        $host = "http://47.242.85.7:9090/sms/batch/v2?appkey={$appkey}&appsecret={$appsecret}&phone={$whole_mobile}&msg=".urlencode($content)."&extend=";
        $result = json_decode(file_get_contents($host));
        if ($result->code == '00000') {
            session(['code' => $verification_code]);
            return $this->success('发送成功');
        } else {
            return $this->error('发送失败' . $result->desc);
        }
         ...
         */
        /* $apikey = "COGNpGhbcKwXWVUA7IvP3Q==";
                $apisecret = "eaaaec38a13549c797fce78a33246fb2";
                $ip =$this-> getRealIp();
                ...
                $send_url = "https://api.i51sms.com/outauth/verifCodeSend";
                $send_data = array(
                    "apikey" => $apikey,
                    "timestamp" => $msg_date,
                    "sign" => $msg_sign,
