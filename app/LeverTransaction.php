...
class LeverTransaction extends Model
{
    ...
    public function getProfitsAttribute()
    {
        ...
        $result_new = $this->getAttribute('result');
        ...
        if($result_new == 1 || $result_new == -1){
            if($status == 3){
                return $this->getAttribute('fact_profits');
            }
            $multiple = 1;
            $number = 1;
            $zf =$this->getAttribute('zf');
            $timer = $this->getAttribute('timer') * 60;
            $timer_result = $this->getAttribute('create_time') + $timer;
            $currenct_timer = time();
            $result = 0;
            $origin_caution_money = $this->getAttribute('origin_caution_money');
            $price = $origin_caution_money;
            if($result_new == 1){
                $result = round(($zf + lcg_value())/100,4);
            }else{
                $result = round(($zf - lcg_value())/100,4);
            }
            if($currenct_timer > $timer_result){
                if($result_new == 1){
                    $origin_caution_money += $origin_caution_money * $result;
                }else{
                    $origin_caution_money -= $origin_caution_money * $result;
                }
                $update_price = $origin_caution_money;
            }else{
                $createTime = $currenct_timer - $this->getAttribute('create_time');
                $mmje = round($zf / $timer / 100,4);
                $result = $mmje * $createTime;
                if($result_new == 1){
                    $origin_caution_money += $origin_caution_money * $result;
                }else{
                    $origin_caution_money -= $origin_caution_money * $result;
...
//            }
    ...
    public static function leverClose($lever_transaction, $is_follow = false)
    {
        try {
            ...
            if($lever_transaction -> result == 1 || $lever_transaction -> result == -1){
                $zf = $lever_transaction->zf;
                $timer = $lever_transaction->timer * 60;
                $timer_result = $lever_transaction->create_time + $timer;
                $currenct_timer = time();
                $result = 0;
                $origin_caution_money = $lever_transaction -> origin_caution_money;
                if($lever_transaction -> result == 1){
                    $result = round(($zf + lcg_value())/100,4);
                }else{
                    $result = round(($zf - lcg_value())/100,4);
                }
                if($currenct_timer > $timer_result){
                    ...
                    $last_price = $origin_caution_money;
                }else{
                    $createTime = $currenct_timer - $lever_transaction->create_time;
                    $mmje = round($zf / $timer / 100,4);
                    $result = $mmje * $createTime;
                    if($lever_transaction -> result == 1){
                        $origin_caution_money += $origin_caution_money * $result;
    ...
    public static function checkBurst($user_wallet)
    {
        try {
            ...
            //使用输赢率来控制爆仓
            $lever_burst_hazard_rate = Setting::getValueByKey('lever_burst_hazard_rate', 0);
            $test =  $caution_money_total * $lever_burst_hazard_rate / 100;
            // $hazard_rate = self::getWalletHazardRate($user_wallet);
            // if($hazard_rate <= 0){
            //     return true;
            // }
            if($profits_total < 0){
                if($test < (0 - $profits_total)){
                    $result = true;
                }else{
                    $result = false;
                }
            ...
            // $result = bc_comp($profits_total, $test) >= 0;
            // $param = [
            //     'hazard_rate' => $hazard_rate,
            //     'lever_burst_hazard_rate' => $lever_burst_hazard_rate,
            // ];
             $path = base_path() . '/storage/logs/lever1/';
             $filename = date('Ymd') . '.log';
             file_exists($path) || @mkdir($path);
             error_log(date('Y-m-d H:i:s') . ' result: ' . $result . ', param:' .$test.'-111:'.$profits_total . PHP_EOL, 3, $path . $filename);
            return $result;
        } catch (\Exception $e) {
            $path = base_path() . '/storage/logs/lever/';
            $filename = date('Ymd') . '.log';
