...
class WsConnection
{
    ...
    protected function onMarketKline($con, $data, $match)
    {
        ...
        if (strtoupper($currency_match->currency_name) == 'BTC' && $currency_match->legal_name == 'USDT') {
            ...
            foreach ($self_match_ids as $id) {
                ...
                if($high_price <=0 || $low_price<=0 || $open_price<=0){
                    continue 1;
                }
                if ($open_price) {
                } else {
        ...
        if ($currency_match->market_from == 2) {
            $legalId = $currency_match -> legal_id;
            $currencyId =$currency_match -> currency_id;
            $total_fix = Db::table('kline_change') -> where("currency_id",$currencyId) -> where('legal_id', $legalId) -> where('status',1) ->sum("fix");
            $tick->open = round($tick->open + $total_fix,5);
            $tick->close = round($tick->close + $total_fix,5);
            $tick->high = round($tick->high + $total_fix,5);
            $tick->low = round($tick->low + $total_fix,5);
            $market_data = [
                'id' => $tick->id,
                'period' => $period,
    ...
    public static function getSelfMatch()
    {
        ...
        if (!$result) {
            //->where('fluctuate_min', '>', 0)->where('fluctuate_max', '>', 0)
            $match_ids = CurrencyMatch::where('market_from', 2)->pluck('id')->toArray();
    ...
    {
        $path = base_path() . '/storage/logs/wss/';
        $filename = date('Ymd') . 'onMatchTrade.log';
        file_exists($path) || @mkdir($path);
        ...
        foreach ($currency_matches as $key => $currency_match) {
            ...
            // }
            // if ($currency_match->market_from == 1) {
                if($currency_match->currency_name == "BTC"){
                    ...
                    foreach ($sys_matches as $k=>$v){
                        ...
                        $self_trade_data = [
                            'type' => 'match_trade',
                            'symbol' => $adt['currency_name'] . '/' . $v["legal_name"],
                            'base-currency' => $adt['currency_name'],
                            'quote-currency' => $v["legal_name"],
                        ...
                        self::$matchTradeData[$adt['currency_name'] . '.' . $v["legal_name"]] = $self_trade_data;
                    }
                }
            // }
        }
    ...
    protected function onMarketDepth($con, $data, $match)
    {
        ...
        if ($currency_match->market_from == 2) {
            ...
            $asks = [];
            foreach ($tick_bids as $bid) {
                $asks[] = [round($bid[0] + $total_fix,5), $bid[1]];
            }
            $bids = [];
            foreach ($tick_asks as $ask) {
                $bids[] = [round($ask[0] + $total_fix,5), $ask[1]];
            }
            $depth_data = [
                'type' => 'market_depth',
                'symbol' => $currency_match->currency_name . '/' . $currency_match->legal_name,
