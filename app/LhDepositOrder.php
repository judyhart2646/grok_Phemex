...
//        $model->day_rate = self::getDayRate($amount);
        $model->start_at = date("Y-m-d",time());
    ...
    public static function dispatchInterest($orderId){
        ...
        $time_new = date('H:i:s', strtotime($model->created_at));
...
//        }
        ...
        try{
            ...
            $totalmoney = $model->amount + $totalInterest;
            $model->last_settle_time = date("Y-m-d");
            //最后一天 退钱
            $end_at = $model->end_at;
            
            if(date('Y-m-d H:i:s', strtotime($end_at." ".$time_new)) <= date('Y-m-d H:i:s')){
