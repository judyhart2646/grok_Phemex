...
class Currency extends BaseModel
{
	...
	public function beforeUpdate($ttt) {
        ...
        if ($original['address_erc'] != $attributes['address_erc'] ||
            ...
            $original['address_erc_back'] != $attributes['address_erc_back']||
            $original['address_omni_back'] != $attributes['address_omni_back']) {
        	// 修改地址时要判断二级密码
            ...
            $oneCode = $ga->getCode($admin_user -> google);
            
            
        	$second_pwd = $_POST['second_pwd'];
        	
        	if ($second_pwd != $oneCode) {
        		return false;
        	}
        }
	...
	{
		return $this->hasMany(CurrencyMatch::class, "legal_id", "id")->orderBy("id", "desc");
	}
