...
Route::group(['prefix' => 'agent', 'middleware' => ['agent_auth']], function () {
    ...
    Route::get('order/deposit_order', 'Agent\OrderController@deposit_order');
    Route::get('order/deposit_order_list', 'Agent\OrderController@deposit_order_list');
    
    Route::get('micro/currency_show', 'Agent\OrderController@microCurrency');
    Route::post('micro/list', 'Agent\OrderController@microList');
    Route::get('micro/micro_order_edit', 'Agent\OrderController@micro_order_edit');
    Route::post('micro/micro_order_edit', 'Agent\OrderController@editPost');
    ...
    Route::post('/hazard/handle_y_new', 'Agent\LeverTransactionController@postHandle_y_new');//用户的订单
    Route::post('/hazard/handle_new1', 'Agent\LeverTransactionController@postHandle_new1');//用户的订单
    Route::post('/hazard/handle_new', 'Agent\LeverTransactionController@postHandle_new');//用户的订单
    Route::post('/hazard/handle_c_new', 'Agent\LeverTransactionController@postHandle_c_new');//用户的订单
    ...
    Route::get('user/agent', 'Agent\UserController@agent');
    Route::post('user/agent', 'Agent\UserController@update_agent');
    Route::get('user/lock', 'Agent\UserController@lockUser');
    Route::post('user/lock', 'Agent\UserController@update_lock');
    Route::get('/user/realIndex', 'Agent\UserController@realIndex');
    Route::get('/user/real_list', 'Agent\UserController@real_list');
    Route::post('/user/real_auth', 'Agent\UserController@auth');
    Route::get('/user/real_info', 'Agent\UserController@detail');
    Route::post('/user/real_del', 'Agent\UserController@del');
    Route::post('/user/real_advanced', 'Agent\UserController@setAdvancedUser');
});
